...
public class Pagination {
	...
	private static final long SIZE = 10;	// 페이지 당 데이터 수 기본 값
	private static final long BLOCKSIZE = 10;	// 노출 데이터 수 기본 값

	// 필수 정보
	private long page;		// 현재 번호
	private long size;		// 페이지 당 데이터 수
	private long blockSize;		// 노출 페이지 수
	...
	public Pagination(long page, long total) {
		this(page, SIZE, BLOCKSIZE, total);
	}

	public Pagination(long page, long size, long blockSize, long total) {
		this.page = page;
		this.size = size;
	...
	public void calc() {
		...
		// 시작 번호
		this.start = ( (page - 1) / blockSize ) * blockSize + 1;

		// 끝 번호
		this.end = ( (page - 1) / blockSize + 1 ) * blockSize;
	...
	public void nomalize() {
		...
		// size
		if ( this.size != 10 && this.size != 20 && this.size != 50 ) {
			this.size = SIZE;
		}
